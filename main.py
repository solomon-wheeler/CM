class node:
    ...
    def get_child_node(self, action, parent):
        ...
        if other_side[0] < other_side[1]: #checking if more cannibales than missionaries on correct side
            if other_side[0] != 0:
                return False
        if new_state[0] < 0 or new_state[1] < 0 or new_state[2] < 0: # checking if we have enough canibals/missionaries on the side to make the move. Boat check should never run, but is useful to catch bugs
            return False
